...
def simulate_sqale_index(training_df, best_model_cfg, simulations=100):
    """
    ...
    simulated_results['Actual'] = actual_sqale_index
...
def assess_simulations(simulated_df):
    ...
    for column in simulated_df.columns:
        if column != 'Actual':
            mape_val = MAPE(simulated_df['Actual'], simulated_df[column])
            ...
            metrics.append([column, mape_val, mse_val, mae_val, rmse_val])
    
    metrics_df = pd.DataFrame(metrics, columns=['Simulation', 'MAPE', 'MSE', 'MAE', 'RMSE'])
    return metrics_df
...
def trigger_simulation(df_path, project_name, periodicity, seasonality):
    ...
    if output_flag:
        ...
        plt.plot(simulated_df['Actual'], label='Actual')
        plt.plot(simulated_df['Simulated_0'], label='Simulated_0')
        plt.legend()
        plt.title('Actual vs Simulated SQALE_INDEX')
        plt.show()
        return [project_name, metrics_df[0], metrics_df[1], metrics_df[2], metrics_df[3]]
    else:
...
def ts_simulation(seasonality):
    ...
    assessment_statistics = ['Simulation', 'MAPE', 'MSE', 'MAE', 'RMSE']
