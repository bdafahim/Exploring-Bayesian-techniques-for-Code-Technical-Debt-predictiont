...
def trigger_simulation(df_path, project_name, periodicity, seasonality,steps):
    ...
    for regressor_combination in combinations:
        ...
        if output_flag:
            if(seasonality):
                combination_results = simulate_sqale_index_sarima_future_points(training_df, testing_df, best_model_cfg, regressor_list, steps, index)
            else:
                combination_results = simulate_sqale_index_arima_future_points(training_df, testing_df, best_model_cfg, regressor_list, steps, index)
        else:
